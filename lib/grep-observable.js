...
var grep = function(jsRE, where, args) {
  ...
  return RxNode.spawn('grep', commandLineArgs, {
    ...
    .doOnError(function(err) {
      var newError = new VError(err, 'grep process failed with error code: %s', err.code);
    ...
    .map(function(grepResultLine) {
      ...
      var file = path.resolve(where, grepResultLineComponents[0]);
    ...
    .doOnError(function(err) {
      var newError = new VError(err, 'Error with grepObservable');
      console.error(newError.stack);
    });
};
