...
function bumpFilesByFindAndReplace(newVersion, filepath, args) {
  ...
  var fs = require('fs');
  var source = grepObservable.grep(versionNumberRE, filepath, args)
    ...
    .concatMap(function(lineDetailsMapAndFile) {
      ...
      function updater(o) {
        ...
        .reduce(function(accumulator, item) {
          return accumulator + '\n' + item;
        // NOTE: we need the empty string here to handle cases
        // when there is only one item in the observable.
        }, '');
      }

      return fileLineUpdater.update(file, updater);
    })
    .subscribe(function(result) {
      subject.onNext(result);
    }, function(err) {
      var newError = new VError(err, 'Error with bumpFilesByFindAndReplace');
      throw newError;
    }, function() {
      subject.onCompleted();
    });
