...
var fileTextByFindAndReplaceSemverBumper = require('./bump-files-by-find-and-replace.js');
var bumpFilesByFindAndReplace = fileTextByFindAndReplaceSemverBumper.bumpFilesByFindAndReplace;
var colors = require('colors');
var fileLineUpdater = require('./update-file-lines.js');
...
function getGrepOptions() {
  ...
  return inquirer.prompt(promptSource).process
    ...
    .doOnError(function(err) {
      var newError = new VError(err, 'Error with getGrepOptions');
      console.error(newError.stack);
    });
}

// Would be more accurate to name it "getJsonFilepaths"
function getJsonFiles() {
  ...
  return inquirer.prompt(promptSource).process
    ...
    .doOnError(function(err) {
      var newError = new VError(err, 'Error with getJsonFiles');
      console.error(newError.stack);
    });
}
...
function getPackageMetadata() {
  var cwd = process.cwd();
  var packageJsonFilepath = path.join(cwd, '/package.json');
    ...
    .concatMap(function(packageMetadata) {
      ...
      return Rx.Observable.if(
        ...
        .concatMap(function(newVersion) {
          ...
          var grepOptions = packageJson.semverInFileTextBumper.grepOptions;
          ...
          .concatMap(function(pair) {
            ...
            grepOptions = pair[1];
            packageJson.semverInFileTextBumper.grepOptions = grepOptions;
            ...
            // TODO it might be working now, but haven't test it yet.
            //return Rx.Observable.empty();
            //*
            var findAndReplaceSource = bumpFilesByFindAndReplace(newVersion, cwd, {
            ...
            var jsonFilesUpdatesSource = jsonFilesSource
              // TODO maybe doOnComplete?
              // how do we know this will run after the grepOptions are set?
              .doOnNext(function(jsonFiles) {
            ...
            //return Rx.Observable.merge(jsonFilesUpdatesSource, findAndReplaceSource);
            return jsonFilesUpdatesSource.concat(findAndReplaceSource);
            //*/
          });
        });
    })
    .doOnError(function(err) {
      var newError = new VError(err, 'Error with semverInFileTextBumper');
      console.error(newError.stack);
    });
};
